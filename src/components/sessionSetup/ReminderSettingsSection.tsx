      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <View style={{ flex: 1 }}>
          <FormField label="Hours">
            <TextInput value={durationHours} onChangeText={onDurationHoursChange} placeholder="0" keyboardType="number-pad" placeholderTextColor={theme.colors.inputPlaceholder} style={formInputStyle} />
          </FormField>
        </View>
        <View style={{ flex: 1 }}>
          <FormField label="Minutes">
            <TextInput value={durationMinutes} onChangeText={onDurationMinutesChange} placeholder="0" keyboardType="number-pad" placeholderTextColor={theme.colors.inputPlaceholder} style={formInputStyle} />
          </FormField>
        </View>
      </View>
    ) : null}
    {plannedEndLabel ? <Text style={{ color: theme.colors.textSoft }}>If you begin now, your planned end time is {plannedEndLabel}.</Text> : null}
    <View style={{ gap: 8 }}>
      <Text style={{ color: theme.colors.text, fontWeight: '700' }}>Reminder Markers</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
        <View style={{ flex: 1 }}>
          <FormField label="Custom Reminder Minute" error={customAlertError}>
            <TextInput value={customAlertMinute} onChangeText={onCustomAlertMinuteChange} placeholder="Custom minute" keyboardType="number-pad" placeholderTextColor={theme.colors.inputPlaceholder} style={formInputStyle} />
          </FormField>
        </View>
      ...
      {reminderValidationMessage ? <Text style={{ color: theme.colors.errorText }}>{reminderValidationMessage}</Text> : null}
      {!!alertMarkersMinutes.length ? <Text style={{ color: theme.colors.textSoft }}>Active reminders: {alertMarkersMinutes.map((minute) => `${minute} min`).join(', ')}</Text> : <Text style={{ color: theme.colors.textSoft }}>No reminders selected. You can use presets or add custom minute markers for your beat.</Text>}
      ...
      {notificationPermissionStatus === 'denied' ? (
        <Text style={{ color: theme.colors.errorText, lineHeight: 20 }}>
